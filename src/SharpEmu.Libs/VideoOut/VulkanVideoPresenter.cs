...
internal static unsafe class VulkanVideoPresenter
{
    ...
    private static readonly ulong _maxPendingGuestWorkBytes =
        (ulong.TryParse(
            ...
            ? pendingGuestWorkMb
            : 256UL) * 1024UL * 1024UL;
    // After the oversized dispatch completes, retain only a bounded number of
    // subsequent work identities for the startup-frontier trace. This keeps
    // lifecycle evidence useful without turning normal frame traffic into a
    // diagnostic log or retaining guest payloads.
    private const int DiagnosticPostCorrectionWorkBudget = 128;
    ...
    private static int _diagnosticPostCorrectionWorkBudget;
    private static long _diagnosticCorrectedDispatchSequence;
    ...
    private static void ResetHostSessionStateLocked()
    {
        ...
        _diagnosticPostCorrectionWorkBudget = 0;
        _diagnosticCorrectedDispatchSequence = 0;
    ...
    private static string DescribeGuestWorkIdentity(object work) => work switch
    {
        ...
        VulkanOffscreenGuestDraw draw =>
            $"offscreen-draw cs=0x{draw.ShaderAddress:X16} " +
            $"targets={draw.Targets.Count}",
        VulkanOffscreenColorClear clear =>
            $"offscreen-clear cs=0x{clear.ShaderAddress:X16} " +
            $"targets={clear.Targets.Count}",
        VulkanGuestImageWrite write =>
            $"image-write addr=0x{write.Address:X16}",
        VulkanOrderedGuestAction => "ordered-action",
        VulkanOrderedGuestFlip => "ordered-flip",
        VulkanOrderedGuestFlipWait => "ordered-flip-wait",
        _ => work.GetType().Name,
    };
    ...
    private static void CompleteGuestWork(in PendingGuestWork pending)
    {
        ...
        lock (_gate)
        {
            ...
            if (completedOversizedWork)
            {
                ...
                _diagnosticPostCorrectionWorkBudget = DiagnosticPostCorrectionWorkBudget;
                MemoryDiagnostics.RecordEvent(
                    "guest-work-frontier",
                    new
                    {
                        eventVersion = 1,
                        phase = "corrected-dispatch-complete",
                        sequence = pending.Sequence,
                        work = DescribeGuestWorkForMemoryDiagnostics(pending.Work),
                        queue = new
                        {
    ...
    private sealed class Presenter : IDisposable
    {
        ...
        private HostMovieFrameLifetime _hostMovieFrameLifetime;
        ...
        private string _diagnosticLastProgress = "not-started";
        private long _diagnosticActiveWorkSequence;
        private ulong _diagnosticActiveSubmissionId;
        private long _diagnosticPhaseStartedTicks;
        private long _diagnosticLastProgressTicks;
        private bool _diagnosticTraceActiveWork;
        private int _diagnosticPendingGuestSubmissionCount;
        private int _diagnosticAbandonedGuestSubmissionCount;
        private int _diagnosticTextureCacheCount;
        private long _diagnosticTextureCacheBytes;
        private long _diagnosticTextureCacheImageBytes;
        private long _diagnosticTextureCacheStagingBytes;
        private long _diagnosticDeferredTextureDestructionBytes;
        private long _diagnosticDeferredTranslatedResourceDestructionBytes;
        private VulkanResourceLifetimeDiagnostics? _resourceLifetimeDiagnostics;
        private List<long>? _diagnosticBatchTextureResourceIds;
        ...
        private sealed class TranslatedDrawResources
        {
            ...
            public long DiagnosticDeferredDeviceBytes;
            public int DiagnosticDeferredOwnership;
        }
        ...
        private sealed class TextureResource
        {
            ...
            public long DiagnosticCacheImageBytes;
            public long DiagnosticCacheStagingBytes;
            public int DiagnosticCacheOwnership;
            public long DiagnosticResourceId;
            public bool DiagnosticUploadRecorded;
            public long DiagnosticDeferredDeviceBytes;
            public int DiagnosticDeferredOwnership;
            public bool IsHostMovie;
            public int HostMoviePlane = -1;
            public long HostMovieGeneration;
            public long HostMovieInstanceId;
            public string? HostMoviePath;
        ...
        private sealed record PendingGuestSubmission(
            ...
            VulkanGuestQueueIdentity Queue,
            long WorkSequence,
            IReadOnlyList<long> DiagnosticTextureResourceIds);
        ...
        private void SetDiagnosticWork(in PendingGuestWork pending)
        {
            ...
            Volatile.Write(
                ...
                pending.Sequence);
            Volatile.Write(
                ref _diagnosticActiveSubmissionId,
                pending.Queue.SubmissionId);
            Volatile.Write(
                ref _diagnosticActiveQueueName,
                pending.Queue.Name);
            Volatile.Write(
                ref _diagnosticActiveWorkSummary,
                DescribeGuestWorkIdentity(pending.Work));
            Volatile.Write(ref _diagnosticTraceActiveWork, pending.DiagnosticTrace);
            SetDiagnosticPhase("guest-work.enter");
        }
        ...
        private void SetDiagnosticPhase(string phase)
        {
            ...
            Volatile.Write(ref _diagnosticPhaseStartedTicks, now);
            Volatile.Write(ref _diagnosticLastProgressTicks, now);
            Volatile.Write(ref _diagnosticLastProgress, phase);

            if (!Volatile.Read(ref _diagnosticTraceActiveWork) ||
                string.Equals(previous, phase, StringComparison.Ordinal))
            {
                return;
            }

            MemoryDiagnostics.RecordEvent(
                "presenter-work-phase",
                new
                {
                    eventVersion = 1,
                    transition = "enter",
                    ...
                    phase,
                    sequence = Volatile.Read(ref _diagnosticActiveWorkSequence),
                    queue = Volatile.Read(ref _diagnosticActiveQueueName),
                    submissionId = Volatile.Read(ref _diagnosticActiveSubmissionId),
                    work = Volatile.Read(ref _diagnosticActiveWorkSummary),
                });
        }
        ...
        private void ClearDiagnosticWork()
        {
            ...
            Volatile.Write(ref _diagnosticActiveSubmissionId, 0);
        ...
        private void RecordTextureDiagnosticUse(
            TextureResource texture,
            bool cacheHit)
        {
            if (!_diagnosticsEnabled ||
                texture.DiagnosticResourceId == 0 ||
                _resourceLifetimeDiagnostics is not { } lifetime)
            {
                return;
            }

            lifetime.RecordUse(
                texture.DiagnosticResourceId,
                cacheHit,
                timeline: 0,
        ...
        private void RecordDiagnosticBatchTexture(TextureResource texture)
        {
            if (!_diagnosticsEnabled ||
                texture.DiagnosticResourceId == 0 ||
                _diagnosticBatchTextureResourceIds is not { } ids ||
                ids.Contains(texture.DiagnosticResourceId))
            {
                return;
            }

            if (ids.Count < VulkanResourceLifetimeDiagnostics.DefaultMaximumEntries)
            {
                ids.Add(texture.DiagnosticResourceId);
            }
        }

        private IReadOnlyList<long> GetDiagnosticResourceIds(
            IReadOnlyList<TranslatedDrawResources> resources)
        {
            if (!_diagnosticsEnabled ||
                _resourceLifetimeDiagnostics is null)
            {
                return [];
            }
            ...
            foreach (var resource in resources)
            {
                ...
                {
                    if (texture.DiagnosticResourceId != 0 &&
                        !ids.Contains(texture.DiagnosticResourceId))
                    {
                        ids.Add(texture.DiagnosticResourceId);
                    }
        ...
        private void TrackCachedTextureDiagnostics(
            GuestDrawTexture descriptor,
            TextureResource resource)
        {
            ...
            var imageBytes = checked((long)VulkanMemoryDiagnostics.GetAllocationSize(
                resource.ImageMemory));
            var stagingBytes = checked((long)VulkanMemoryDiagnostics.GetAllocationSize(
                resource.StagingMemory));
            resource.DiagnosticCacheImageBytes = imageBytes;
            resource.DiagnosticCacheStagingBytes = stagingBytes;
            resource.DiagnosticCacheDeviceBytes = checked(imageBytes + stagingBytes);
            resource.DiagnosticCacheOwnership = 1;
            resource.DiagnosticResourceId = lifetime.TrackCacheInsertion(
                new VulkanTextureDiagnosticDescriptor(
                    descriptor.Address,
                    descriptor.Width,
                    descriptor.Height,
                    resource.Depth,
                    ...
                    resource.Layers,
                    descriptor.Type,
                    descriptor.ArrayedView,
                    descriptor.IsStorage),
                _activeGuestWorkSequence,
                _activeGuestQueue.Name,
                MemoryDiagnostics.GetElapsedMilliseconds(),
                imageBytes,
                stagingBytes,
                stagingMapped: false,
                uploadRecorded: resource.DiagnosticUploadRecorded);

            if (resource.DiagnosticUploadRecorded)
            {
                RecordDiagnosticBatchTexture(resource);
            }

            if (resource.DiagnosticResourceId == 0)
            {
                resource.DiagnosticCacheOwnership = 0;
                resource.DiagnosticCacheDeviceBytes = 0;
                resource.DiagnosticCacheImageBytes = 0;
                resource.DiagnosticCacheStagingBytes = 0;
                return;
            }

            Interlocked.Increment(ref _diagnosticTextureCacheCount);
            Interlocked.Add(ref _diagnosticTextureCacheBytes, resource.DiagnosticCacheDeviceBytes);
            Interlocked.Add(ref _diagnosticTextureCacheImageBytes, imageBytes);
            Interlocked.Add(ref _diagnosticTextureCacheStagingBytes, stagingBytes);
            MemoryDiagnostics.Adjust(
                "vulkan.texture-cache-retained",
                resource.DiagnosticCacheDeviceBytes,
                countDelta: 1);
            MemoryDiagnostics.Adjust(
                "vulkan.texture-cache-image",
                imageBytes,
                countDelta: 1);
            MemoryDiagnostics.Adjust(
                "vulkan.texture-cache-staging",
                stagingBytes,
                countDelta: stagingBytes == 0 ? 0 : 1);
        }

        private static long GetTranslatedResourceDiagnosticBytes(
            TranslatedDrawResources resources)
        {
            ...
            {
                if (texture.Cached || texture.GuestImage is not null)
                {
                    continue;
                }
                ...
                {
                    handles.Add(texture.ImageMemory.Handle);
                }

                if (texture.StagingMemory.Handle != 0)
                {
                    handles.Add(texture.StagingMemory.Handle);
                }
            }

            foreach (var vertex in resources.VertexBuffers)
            {
                if (vertex.OwnsBuffer && vertex.Memory.Handle != 0)
                {
                    handles.Add(vertex.Memory.Handle);
                }
            }

            if (resources.IndexMemory.Handle != 0)
            {
                handles.Add(resources.IndexMemory.Handle);
            }

            foreach (var global in resources.GlobalMemoryBuffers)
            {
                if (global.Allocation is null && global.Memory.Handle != 0)
                {
                    handles.Add(global.Memory.Handle);
                }
            }
            ...
            {
                bytes = checked(bytes + (long)VulkanMemoryDiagnostics.GetAllocationSize(
                    new DeviceMemory(handle)));
            }

            return bytes;
        }

        private void RecordDeferredTranslatedResource(
            TranslatedDrawResources resources,
            ulong retireTimeline)
        {
            if (!_diagnosticsEnabled ||
                resources.DiagnosticDeferredOwnership != 0)
            {
                return;
            }

            var bytes = GetTranslatedResourceDiagnosticBytes(resources);
            resources.DiagnosticDeferredDeviceBytes = bytes;
            resources.DiagnosticDeferredOwnership = 1;
            Interlocked.Add(ref _diagnosticDeferredTranslatedResourceDestructionBytes, bytes);
            MemoryDiagnostics.Adjust(
                "vulkan.deferred-translated-resource-destruction",
                bytes);
            MemoryDiagnostics.RecordEvent(
                "vulkan-translated-resource-lifetime",
                new
                {
                    action = "deferred",
                    bytes,
                    retireTimeline,
                    guestWorkSequence = _activeGuestWorkSequence,
                });
        }

        private void RecordActualTranslatedResourceDestroy(
            TranslatedDrawResources resources,
            ulong timeline)
        {
            if (!_diagnosticsEnabled ||
                resources.DiagnosticDeferredOwnership == 0 ||
                Interlocked.Exchange(ref resources.DiagnosticDeferredOwnership, 0) == 0)
            {
                return;
            }

            var bytes = resources.DiagnosticDeferredDeviceBytes;
            resources.DiagnosticDeferredDeviceBytes = 0;
            Interlocked.Add(ref _diagnosticDeferredTranslatedResourceDestructionBytes, -bytes);
            MemoryDiagnostics.Adjust(
                "vulkan.deferred-translated-resource-destruction",
                -bytes);
        ...
        private object? GetDiagnosticSnapshot()
        {
            ...
            var lastProgress = Volatile.Read(ref _diagnosticLastProgressTicks);
            var waits = GpuWaitRegistry.SnapshotOutstanding();
            var hostBuffers = _hostBufferPool.GetDiagnosticsSnapshot();
            List<object> queueDepths;
            int pendingGuestWorkCount;
            int pendingPayloadGuestWorkCount;
            int pendingSyncGuestWorkCount;
            ulong pendingGuestWorkBytes;
            long completedGuestWorkSequence;
            lock (_gate)
            {
                pendingGuestWorkCount = _pendingGuestWorkCount;
                pendingPayloadGuestWorkCount = _pendingPayloadGuestWorkCount;
                pendingSyncGuestWorkCount = _pendingSyncGuestWorkCount;
                pendingGuestWorkBytes = _pendingGuestWorkBytes;
                completedGuestWorkSequence = _completedGuestWorkSequence;
                queueDepths = _pendingGuestWorkByQueue
                    .Select(static entry => (Name: entry.Key, Depth: entry.Value.Count,
                        Head: entry.Value.First?.Value.Sequence ?? 0))
                    .OrderByDescending(static entry => entry.Depth)
            ...
            return new
            {
                presenter = new
                {
                    initialized = Volatile.Read(ref _vulkanReady),
                    deviceLost = Volatile.Read(ref _deviceLost),
                    phase,
                    phaseAgeMilliseconds = DiagnosticAgeMilliseconds(now, phaseStarted),
                    lastProgress = Volatile.Read(ref _diagnosticLastProgress),
                    lastProgressAgeMilliseconds = DiagnosticAgeMilliseconds(now, lastProgress),
                    activeWork = new
                    {
                        sequence = Volatile.Read(ref _diagnosticActiveWorkSequence),
                        queue = Volatile.Read(ref _diagnosticActiveQueueName),
                        submissionId = Volatile.Read(ref _diagnosticActiveSubmissionId),
                        summary = Volatile.Read(ref _diagnosticActiveWorkSummary),
                    },
                    lastGuestWork = Volatile.Read(ref _lastGuestWorkLabel),
                    lastSubmit = Volatile.Read(ref _lastSubmitDebugName),
                    milestones = new
                    {
                        splashPresented = Volatile.Read(ref _splashPresented),
                        firstFramePresented = Volatile.Read(ref _firstFramePresented),
                        firstGuestDrawPresented = Volatile.Read(ref _firstGuestDrawPresented),
                        presentedSequence = Volatile.Read(ref _presentedSequence),
                    },
                },
                guestQueue = new
                {
                    pendingCount = pendingGuestWorkCount,
                    pendingPayloadCount = pendingPayloadGuestWorkCount,
                    pendingSyncCount = pendingSyncGuestWorkCount,
                    pendingBytes = pendingGuestWorkBytes,
                    completedContiguousSequence = completedGuestWorkSequence,
                    executingSequence = Volatile.Read(ref _executingGuestWorkSequence),
                    correctedDispatchSequence = Volatile.Read(ref _diagnosticCorrectedDispatchSequence),
                    postCorrectionWorkBudget = Volatile.Read(ref _diagnosticPostCorrectionWorkBudget),
                    queueDepths,
                },
                gpu = new
                {
                    pendingSubmissions = Volatile.Read(ref _diagnosticPendingGuestSubmissionCount),
                    abandonedSubmissions = Volatile.Read(ref _diagnosticAbandonedGuestSubmissionCount),
                    submitTimeline = Volatile.Read(ref _submitTimeline),
                    completedTimeline = Volatile.Read(ref _completedTimeline),
                    batchOpen = Volatile.Read(ref _batchOpen),
                    batchDrawCount = Volatile.Read(ref _batchDrawCount),
                    batchRetireBufferCount = _batchRetireBuffers.Count,
                    batchRetireDetileCount = _batchRetireDetile.Count,
                    detileCount = Volatile.Read(ref _gpuDetileCount),
                },
                resources = new
                {
                    textureCacheCount = Volatile.Read(ref _diagnosticTextureCacheCount),
                    textureCacheDeviceBytes = Volatile.Read(ref _diagnosticTextureCacheBytes),
                    textureCacheImageBytes = Volatile.Read(ref _diagnosticTextureCacheImageBytes),
                    textureCacheStagingBytes = Volatile.Read(ref _diagnosticTextureCacheStagingBytes),
                    deferredTextureDestructionBytes =
                        Volatile.Read(ref _diagnosticDeferredTextureDestructionBytes),
                    deferredTranslatedResourceDestructionBytes =
                        Volatile.Read(ref _diagnosticDeferredTranslatedResourceDestructionBytes),
                    hostBufferCachedBytes = hostBuffers.CachedBytes,
                    hostBufferCachedAllocations = hostBuffers.CachedAllocations,
                ...
                guestWait = new
                {
                    ...
                    latched = waits.Latched,
                    oldestAgeMilliseconds = waits.OldestAgeMs,
                    sampleWaitAddress = waits.SampleWaitAddress,
                    sampleQueue = waits.SampleQueueName,
                },
            };
        }
            ...
            try
            {
                ...
                SubmitGuestCommandBuffer(
                    ...
                    retireDetile: _batchRetireDetile.Count > 0
                        ...
                        : [],
                    diagnosticTextureResourceIds:
                        _diagnosticBatchTextureResourceIds is { Count: > 0 } ids
                            ? ids.ToArray()
                            : []);
            }
            catch
        ...
        private void SubmitGuestCommandBuffer(
            ...
            IReadOnlyList<long>? diagnosticTextureResourceIds = null)
        {
            ...
            if (_diagnosticsEnabled && _resourceLifetimeDiagnostics is { } lifetime)
            {
                var ids = new List<long>();
                ...
                if (diagnosticTextureResourceIds is not null)
                {
                    foreach (var resourceId in diagnosticTextureResourceIds)
                    {
                        if (resourceId != 0 && !ids.Contains(resourceId))
                        {
                            ids.Add(resourceId);
                        }
                    }
                ...
                {
                    lifetime.RecordSubmission(resourceId, _submitTimeline);
                }

                submittedDiagnosticTextureIds = ids.Count == 0 ? [] : ids.ToArray();
            }
            foreach (var referenced in referencedResources ?? resources)
            {
        ...
        private void EnsureGuestSubmissionCapacity()
        {
            ...
            {
                SetDiagnosticPhase("gpu.submission-capacity-wait");
        ...
        private void CollectCompletedGuestSubmissions(bool waitForOldest, ulong maxWaitNs = 0)
        {
            ...
            {
                SetDiagnosticPhase("gpu.guest-fence-wait");
                var fence = oldest.Fence;
                ...
                {
                    SetDiagnosticPhase("gpu.guest-fence-timeout");
        ...
        private void RetireGuestSubmission(PendingGuestSubmission submission)
        {
            ...
            if (_diagnosticsEnabled && _resourceLifetimeDiagnostics is { } lifetime)
            {
                foreach (var resourceId in submission.DiagnosticTextureResourceIds)
                {
                    lifetime.RecordUploadCompletion(resourceId, submission.Timeline);
                }
            }
        ...
        private void ProcessDeferredTextureDestroys()
        {
                   ...
                   entry.RetireTimeline <= _completedTimeline)
            {
                ...
                DestroyCachedTextureResource(entry.Texture, entry.RetireTimeline);
            }
                   ...
                   resourceEntry.RetireTimeline <= _completedTimeline)
            {
                ...
                RecordActualTranslatedResourceDestroy(
                    resourceEntry.Resources,
                    resourceEntry.RetireTimeline);
        ...
        private void PumpHostMovieFrame()
        {
            if (!HostMovieBridge.TryDecodeNextFrame(
                    ...
                    out var movieGeneration))
            {
                InvalidateInactiveHostMovieFrame("pump-no-frame");
                if (MovieDiagnostics.Enabled)
                {
            ...
            // A close or replacement can race the return from the bridge lock.
            // Do not publish a frame after its generation has ended, even if
            // the decoder handed us pixels before the close acquired the lock.
            var activeGeneration = HostMovieBridge.ActiveHostMovieGeneration;
            if (activeGeneration != movieGeneration ||
                !HostMovieBridge.IsHostMovieGenerationActive(movieGeneration))
            {
                InvalidateInactiveHostMovieFrame("pump-inactive-generation");
                return;
            }

            var generationChanged = _hostMovieFrameLifetime.Publish(movieGeneration);
            if (generationChanged)
            {
                ResetHostMovieBindingState();
                _hostMovieFramePixels = null;
                _hostMovieLumaPixels = null;
                _hostMovieChromaPixels = null;
            ...
            if (!generationChanged && !advanced && _hostMovieFramePixels is not null)
            {
                return;
            }
        ...
        private void InvalidateInactiveHostMovieFrame(string reason)
        {
            var activeGeneration = HostMovieBridge.ActiveHostMovieGeneration;
            var frameGeneration = _hostMovieFrameLifetime.Generation;
            if (!_hostMovieFrameLifetime.HasFrame ||
                frameGeneration == activeGeneration)
            {
                return;
            }

            var path = _hostMovieFramePath;
            var instanceId = _hostMovieFrameInstanceId;
            var frameSerial = _hostMovieFrameSerial;
            _hostMovieFrameLifetime.Invalidate(frameGeneration);
            if (MovieDiagnostics.Enabled)
            {
                MovieDiagnostics.PresenterInvalidation(
                    path,
                    instanceId,
                    frameGeneration,
                    activeGeneration,
        ...
        private void ResetHostMovieBindingState()
        {
            ...
            _tracedHostMovieTextureBindings.Clear();
        }

        private void ResetHostMovieFrameState()
        {
            ResetHostMovieBindingState();
            _hostMovieFrameLifetime.Reset();
            _hostMovieFramePixels = null;
            _hostMovieLumaPixels = null;
            _hostMovieChromaPixels = null;
            _hostMovieFrameWidth = 0;
            ...
            int plane)
        {
            ...
            if (!_hostMovieFrameLifetime.IsEligible(activeGeneration))
            {
                InvalidateInactiveHostMovieFrame("resource-creation");
                return ResolveTextureResource(texture);
            }

            var hostMovieGeneration = _hostMovieFrameLifetime.Generation;
            ...
            return new TextureResource
            {
                ...
                HostMovieGeneration = hostMovieGeneration,
                HostMovieInstanceId = _hostMovieFrameInstanceId,
                HostMoviePath = _hostMovieFramePath,
        ...
        private TextureResource GetOrCreateCachedTextureResource(GuestDrawTexture texture)
        {
            ...
            {
                RecordTextureDiagnosticUse(cached, cacheHit: true);
                return cached;
            }
            ...
            if (resource.OwnsStorage)
            {
                ...
                if (_diagnosticsEnabled)
                {
                    TrackCachedTextureDiagnostics(texture, resource);
                }
            ...
            RecordTextureDiagnosticUse(resource, cacheHit: false);
        ...
        private void DrainGuestImageCpuSync()
        {
            ...
            if (_textureCache.Count > 2048)
            {
                foreach (var entry in _textureCache)
                {
                    RecordTextureCacheRemoval(entry.Value, "cache-size-limit", retireTimeline);
                    EnqueueDeferredTextureDestroy(entry.Value, retireTimeline);
                    ReleaseTextureCacheDiagnosticOwnership(entry.Value);
            ...
            else if (evicted is not null)
            {
                foreach (var key in evicted)
                {
                    ...
                    {
                        RecordTextureCacheRemoval(resource, "guest-write", retireTimeline);
                        EnqueueDeferredTextureDestroy(resource, retireTimeline);
                        ReleaseTextureCacheDiagnosticOwnership(resource);
        ...
        private void RecordTextureCacheRemoval(
            TextureResource texture,
            string reason,
            ulong retireTimeline)
        {
        ...
        private void EnqueueDeferredTextureDestroy(
            TextureResource texture,
            ulong retireTimeline)
        {
            if (!_diagnosticsEnabled ||
                texture.DiagnosticResourceId == 0 ||
                _resourceLifetimeDiagnostics is not { } lifetime ||
                Interlocked.Exchange(ref texture.DiagnosticDeferredOwnership, 1) != 0)
            {
                return;
            }

            var bytes = checked(
                texture.DiagnosticCacheImageBytes +
                texture.DiagnosticCacheStagingBytes);
            texture.DiagnosticDeferredDeviceBytes = bytes;
            Interlocked.Add(ref _diagnosticDeferredTextureDestructionBytes, bytes);
            MemoryDiagnostics.Adjust(
                "vulkan.deferred-texture-destruction",
                bytes);
            lifetime.RecordDeferredDestroy(
                texture.DiagnosticResourceId,
                retireTimeline,
                MemoryDiagnostics.GetElapsedMilliseconds());
        }

        private void RecordActualTextureDestroy(
            TextureResource texture,
            ulong retireTimeline)
        {
            if (!_diagnosticsEnabled ||
                texture.DiagnosticResourceId == 0 ||
                _resourceLifetimeDiagnostics is not { } lifetime ||
                Interlocked.Exchange(ref texture.DiagnosticDeferredOwnership, 0) == 0)
            {
                return;
            }

            var bytes = texture.DiagnosticDeferredDeviceBytes;
            texture.DiagnosticDeferredDeviceBytes = 0;
            Interlocked.Add(ref _diagnosticDeferredTextureDestructionBytes, -bytes);
            MemoryDiagnostics.Adjust(
                "vulkan.deferred-texture-destruction",
                -bytes);
            lifetime.RecordActualDestroy(
                texture.DiagnosticResourceId,
                retireTimeline,
                MemoryDiagnostics.GetElapsedMilliseconds(),
                "deferred-retire");
        ...
        private void DestroyCachedTextureResource(
            TextureResource texture,
            ulong retireTimeline = 0)
        {
            ReleaseTextureCacheDiagnosticOwnership(texture);
            ...
            RecordActualTextureDestroy(texture, retireTimeline);
        }
        ...
        private void ReleaseTextureCacheDiagnosticOwnership(TextureResource texture)
        {
            ...
            var imageBytes = texture.DiagnosticCacheImageBytes;
            var stagingBytes = texture.DiagnosticCacheStagingBytes;
            texture.DiagnosticCacheImageBytes = 0;
            texture.DiagnosticCacheStagingBytes = 0;
        ...
        private TextureResource CreateTextureResource(GuestDrawTexture texture)
        {
            ...
            var resource = new TextureResource
            {
                ...
                OwnsStorage = true,
                DiagnosticUploadRecorded = gpuDetiled,
                SamplerState = texture.Sampler,
            ...
            string debugName)
        {
            ...
            Check(
                VulkanMemoryDiagnostics.Map(
                    _vk, _device, memory, 0, size, 0, &mapped, "texture-staging"),
                "vkMapMemory(texture)");
            ...
            string label = "buffer")
        {
            ...
            Check(
                VulkanMemoryDiagnostics.Allocate(
                    _vk, _device, &memoryInfo, out memory, label),
                "vkAllocateMemory");
        ...
        {
            SetDiagnosticPhase("compute.execute");
        ...
        {
            SetDiagnosticPhase("compute.flush-before");
            FlushBatchedGuestCommands();
            if (_deviceLost)
            {
                return;
            ...
            SetDiagnosticPhase("compute.host-movie");
            PumpHostMovieFrame();
            ...
            SetDiagnosticPhase("compute.validate");
            ...
            try
            {
                SetDiagnosticPhase("compute.submission-capacity");
                EnsureGuestSubmissionCapacity();
                SetDiagnosticPhase("compute.resource-create");
                resources = CreateComputeDispatchResources(work);

                SetDiagnosticPhase("compute.flush-before-submit");
                FlushBatchedGuestCommands();
                ...
                for (var batchIndex = 0u; batchIndex < batchCount; batchIndex++)
                {
                    ...
                    SetDiagnosticPhase("compute.command-record");
                    ...
                    SetDiagnosticPhase("compute.command-submit");
                    if (isLastBatch)
                    {
                        SubmitGuestCommandBuffer(
                            commandBuffer,
                ...
                SetDiagnosticPhase("compute.mark-initialized");
            ...
            finally
            {
                ...
                if (!submitted && resources is not null)
                {
                    if (chunksSubmitted > 0)
                    {
                        ...
                        RecordDeferredTranslatedResource(resources, _submitTimeline);
        ...
        private void RenderCore()
        {
            ...
            {
                SetDiagnosticPhase("close-requested");
            ...
            if (!_vulkanReady)
            {
                SetDiagnosticPhase("vulkan-init");
                return;
            }

            if (_deviceLost)
            {
                SetDiagnosticPhase("device-lost-drain");
            ...
            bool frameSlotReady;
            if (_frameFencePending.Length > frameSlot && _frameFencePending[frameSlot])
            {
                SetDiagnosticPhase("frame-slot-wait");
            }
            ...
            SetDiagnosticPhase("presenter.collect");
            ...
            {
                SetDiagnosticPhase("guest-work.take");
                ...
                if (!tookGuestWork)
                {
                    SetDiagnosticPhase("presentation.take");
                    break;
                }
                ...
                if (!string.Equals(
                        ...
                        StringComparison.Ordinal))
                {
                    SetDiagnosticPhase("guest-work.queue-switch-flush");
                    FlushBatchedGuestCommands();
                }
            ...
            {
                SetDiagnosticPhase("presenter.flush-batch");
                FlushBatchedGuestCommands();
            }
            ...
            if (!tookPresentation)
            {
                SetDiagnosticPhase("presentation.idle");
            ...
            var presentedHostMovieGeneration = 0L;
            var presentedHostMovieInstanceId = 0L;
            string? presentedHostMoviePath = null;
            if (translatedResources is not null && MovieDiagnostics.Enabled)
            {
                foreach (var texture in translatedResources.Textures)
                {
                    ...
                    presentedHostMovieGeneration = texture.HostMovieGeneration;
                    presentedHostMovieInstanceId = texture.HostMovieInstanceId;
                    presentedHostMoviePath = texture.HostMoviePath;
            ...
            SetDiagnosticPhase("presentation.record");
            ...
            SetDiagnosticPhase("presentation.complete");
            ...
            PipelineStageFlags shaderStage)
        {
            ...
            foreach (var texture in resources.Textures)
            {
                ...
                if (_diagnosticsEnabled &&
                    texture.DiagnosticResourceId != 0 &&
                    _resourceLifetimeDiagnostics is { } lifetime)
                {
                    texture.DiagnosticUploadRecorded = true;
                    lifetime.RecordUploadRecorded(texture.DiagnosticResourceId);
                }
                ...
                if (texture.IsHostMovie)
                {
                    ...
                    if (texture.HostMoviePlane == 0)
                    {
                        ...
                        if (isCurrentHostMovieGeneration)
                        {
        ...
        private void RecordTextureCacheDeviceTeardownDiagnostics()
        {
            if (!_diagnosticsEnabled ||
                _resourceLifetimeDiagnostics is not { } lifetime)
            {
            ...
            var timestamp = MemoryDiagnostics.GetElapsedMilliseconds();
        ...
        private void DisposeVulkan()
        {
            ...
            RecordTextureCacheDeviceTeardownDiagnostics();
